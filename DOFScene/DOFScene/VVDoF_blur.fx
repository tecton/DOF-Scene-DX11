...
{
	float4 nearResult: SV_TARGET0;
	float4 blurResult: SV_TARGET1;
};
...
PixelOutputType PS(PS_IN input)
{
    ...
    float2 A = float2(gl_FragCoord.xy) * (direction + ivec2(1));
    ...
    for (int delta = -maxCoCRadiusPixels; delta <= maxCoCRadiusPixels; ++delta)	{
        ...
        // Packed values
        float4 blurInput = texelFetch(blurSourceBuffer, clamp(B, float2(0), textureSize(blurSourceBuffer, 0) - float2(1)), 0);
