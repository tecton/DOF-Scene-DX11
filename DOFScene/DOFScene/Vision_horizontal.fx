...
Texture2D visionCocBuffer : register (t1);
...
PixelOutputType PS(PS_IN input)
{
	...
	float4 cocBuffer = visionCocBuffer.Sample(colorSampler, A);
    float packedA = cocBuffer.r;
    ...
    for (int delta = -maxCoCRadiusPixels; delta <= maxCoCRadiusPixels; ++delta)	{
        ...
        float r_B = (visionCocBuffer.Sample(colorSampler, B).r * 2.0 - 1.0) * float(maxCoCRadiusPixels);
    ...
    result.blurResult.a = cocBuffer.g;
